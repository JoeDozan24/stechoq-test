module.exports = {
    ...
    create: function(conn, data, callback) {
        conn.query("INSERT INTO program_studi SET kode_program_studi = ${data.kode_program_studi}, nama = ${data.nama}, SKS = ${data.sks}, kode = ${data.kode}", callback)
    },
    
    update: function(conn, data, id, callback) {
        conn.query("UPDATE program_studi SET kode_program_studi = ${data.kode_program_studi}, nama = ${data.nama}, SKS = ${data.sks}, kode = ${data.kode} WHERE kode_program_studi = ${id}", callback)
    },
